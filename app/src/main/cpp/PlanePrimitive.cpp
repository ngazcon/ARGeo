...
namespace argeo
{
	...
	PlanePrimitive::PlanePrimitive(
		...
		, m_translated_model_matrix(mat_translate(m_origin))
		, m_rotation(quaternion::identity())
		, m_scale(vec3d(1.0, 1.0, 1.0))
		, m_model_matrix_changed(false)
		, m_translated_model_matrix_changed(false)
	{
	...
	void PlanePrimitive::update(FrameState* frame_state)
	{
		...
		if (m_show)
		{
			if (frame_state->get_pass() == Pass::Render)
			{
				...
				const mat4d model_matrix = get_translated_model_matrix();
				...
				if (m_has_outline && get_is_picked())
				{
					...
					m_outline_command->set_model_matrix(model_matrix * scale_matrix);
	...
	mat4d PlanePrimitive::get_translated_model_matrix()
	{
		if (m_translated_model_matrix_changed || m_model_matrix_changed)
		{
			m_translated_model_matrix = mat_translate(m_origin) * get_model_matrix();
			m_translated_model_matrix_changed = false;
		}

		return m_translated_model_matrix;
	}
	...
	void PlanePrimitive::update_model_matrix()
	{
		if (m_model_matrix_changed)
		{
			...
			m_model_matrix_changed = false;
			m_translated_model_matrix_changed = true;
		}
	}
