...
namespace argeo
{
        ...
        */
		...
		for (unsigned int i = 0; i < num_frustums; i++)
		{
            ...
            */
			...
			{
				m_terrain_depth->update(m_context, m_viewport);
            ...
            */
                ...
                (m_use_depth_picking))
			{
                ...
                //Texture2D* depth_stencil_texture = //depth_only ?
                   //static_cast<Texture2D*>(pass_state->get_framebuffer()->get_depth_stencil_attachment_texture()) :
        ...
        const unsigned int& height)
	{
		...
		std::thread thread(
            ...
            (Bitmap* reality_snapshot, Camera* camera)
		*/
		//{
			...
			if (reality_snapshot == nullptr)
			{
				...
				BitmapAlgorithms::rgba_ubyte_argb_int32(
					reinterpret_cast<unsigned char*>(rgba.data()),
					argb.data(),
					width,
					height,
					true);

				reality_snapshot->set_pixels(argb.data());
			}
			...
			Bitmap* ttar_snapshot = new Bitmap(
				width,
				height,
				PixelFormat::Format32bppArgb);
			ttar_snapshot->set_pixels(argb.data());
	...
	void Scene::take_ar_buffer_snapshot()
	{
			...
			[&, width, height, reality_downloader_index, mixed_reality_downloader_index, virtuality_downloader_index]
		{
			...
			Bitmap* virtuality_snapshot = new Bitmap(
				...
				height,
				PixelFormat::Format32bppArgb);
			virtuality_snapshot->set_pixels(argb.data());
			...
			Bitmap* mixed_reality_snapshot = new Bitmap(
				...
				height,
				PixelFormat::Format32bppArgb);
			mixed_reality_snapshot->set_pixels(argb.data());
