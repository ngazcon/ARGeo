...
namespace argeo
{
	...
	public:
		...
		///
		OrthographicFrustum(
			const double& aspect_ratio = std::numeric_limits<double>::quiet_NaN(),
            const double& width = std::numeric_limits<double>::quiet_NaN(),
            const double& near    = 0,
        ...
        OrthographicOffCenterFrustum* get_offcenter_frustum();
	...
	private:
		...
		std::unique_ptr<OrthographicOffCenterFrustum> m_offcenter_frustum;
		double m_near;
		double m_far;
